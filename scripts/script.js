...
function getSupabase() {
  try {
    ...
    if (!window.__supa) window.__supa = window.supabase.createClient(window.SUPABASE_URL, window.SUPABASE_ANON_KEY);
...
async function saveComments(slug, list) {
  const last = list && list[list.length - 1];
  const client = getSupabase();
  if (client && last) {
    try {
      await client.from('comments').insert([{ slug, name: last.name, text: last.text, ts: last.ts || Date.now() }]);
      // no return value
      return;
    } catch (e) {
