...
const supa = (window.supabase && SB_URL && SB_ANON)
  ...
  : null;
if (!supa) { console.warn('Supabase 未初始化：请检查 index.html 中的 SB_URL / SB_ANON 与 SDK 引入顺序'); }
// ---------------------------------
// 表单校验与反馈
(() => {
...
async function fetchCommentsFromCloud(slug) {
  ...
  return (data || []).map(d => ({ name: d.name, text: d.text, ts: Date.parse(d.created_at) || Date.now() }));
}

async function addCommentToCloud(slug, { name, text }) {
  if (!supa) throw new Error('Supabase 未就绪');
...
async async function renderComments(slug) {
  const container = $("#comment-list");
  if (!container) return;
  const list = await fetchCommentsFromCloud(slug);
  if (!list || list.length === 0) { container.innerHTML = `<div class="text-muted">还没有评论，快来抢沙发～</div>`; return; }
