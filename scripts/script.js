// scripts/script.js
// SPA 路由 + 动态文章渲染 + 评论功能（Supabase 云端版）

const $ = (sel, root = document) => root.querySelector(sel);
const $$ = (sel, root = document) => Array.from(root.querySelectorAll(sel));

// ---------- Supabase 客户端（云端评论） ----------
const sb = (window.supabase && window.SB_URL && window.SB_ANON)
...
// --- 云端评论（替换原 localStorage 实现） ---
async function fetchCommentsFromCloud(slug) {
  if (!sb) return [];
  ...
  if (error) { console.error('[comments select]', error); return []; }
  return data.map(r => ({ name: r.name, text: r.text, ts: Date.parse(r.created_at) || Date.now() }));
}

async function addCommentToCloud(slug, { name, text }) {
  if (!sb) throw new Error('Supabase 未初始化（请设置 SB_URL / SB_ANON）');
  const { error } = await sb.from('comments').insert({ slug, name, text });
...
async function renderComments(slug) {
  ...
  if (!container) return;
  const list = await fetchCommentsFromCloud(slug);
...
function initCommentForm(slug) {
  ...
  form.onsubmit = async (e) => {
    ...
    try {
      ...
      await addCommentToCloud(slug, { name, text });
